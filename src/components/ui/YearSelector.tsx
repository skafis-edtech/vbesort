...
const YearSelector: React.FC<YearSelectorProps> = ({
    ...
    const selectWithAns = () => {
        setYearList(
            allYearList.filter((year) => !noAnsYearList.includes(year))
        );
    };
                                    ...
                                    {allYearList.map((year) => {
                                        ...
                                        return (
                                            <Form.Check
                                                ...
                                                key={year}
                                                inline
                                                label={yearLabelStringify(
                                                    yearNumber,
                                                    session
                                                )}
                                                checked={yearList.includes(
                                                    year
                                                )}
                                                onChange={() =>
                                                    toggleYearInList(year)
                                                }
                                            />
                                        );
                                    })}
